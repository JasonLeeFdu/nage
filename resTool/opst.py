...
def dropout(x,is_training,name='dropout'):
    with tf.name_scope(name):
        kb = tf.cond(is_training,lambda: tf.constant(0.5),lambda: tf.constant(1.0))
        res = tf.nn.dropout(x,kb)
    return  res
...
def resblockSpecial1(x_init, channels, is_training=True, use_bias=True, downsample=False, scope='resblock'):
    with tf.variable_scope(scope):
        ...
        x = relu(x)
        x = conv(x, channels, kernel=3, stride=2, use_bias=use_bias, scope='conv_0')
        x_init = conv(x_init, channels * 2, kernel=1, stride=2, use_bias=use_bias, scope='conv_init')
        x_init = tf.nn.max_pool(x_init,[1,2,2,1],strides=[1,2,2,1],padding='SAME')
        x = batch_norm(x, is_training, scope='batch_norm_1')
        x = relu(x)
